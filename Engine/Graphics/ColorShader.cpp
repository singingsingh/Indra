...
namespace Engine
{
	namespace Graphics
	{
		...
		bool ColorShader::Initialize(ID3D11Device *device)
		{
			...
			result = InitializeShader(device, "colorShaderVS.hlsl", "colorShaderPS.hlsl");
			if (!result)
			{
				return false;
			}
		...
		bool ColorShader::InitializeShader(ID3D11Device *device, const char  *vsFilename, const char  *psFilename)
		{
			...
			if (FAILED(result))
			{
				...
				{
					MessageBox(System::Window::GetWindwsHangle(), vsFilename, "Missing Shader File", MB_OK);
				}
			...
			if (FAILED(result))
			{
				...
				{
					MessageBox(System::Window::GetWindwsHangle(), psFilename, "Missing Shader File", MB_OK);
				}
		...
		void ColorShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, const char * shaderFilename)
		{
			...
			MessageBox(System::Window::GetWindwsHangle(), "Error compiling shader.  Check shader-error.txt for message.", shaderFilename, MB_OK);
